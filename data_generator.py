...
def single_disease_map_label(disease_map_size, RF_size, stride, gt_box, thr):
    '''
    ...
    for box in gt_box:
        ...
        radius = cal_distance(x1, x2, y1, y2) / (2 * stride)
    ...
    for y in range(row):
        for x in range(col):
            for center in center_list:
                ...
                if cal_distance(x, center_x, y, center_y) <= radius:
                    label_map[y][x] += gaussian_kernel(abs(x - center_x), abs(y - center_y), (1 / 3) * radius)

    return label_map


def get_disease_map_label(gt_box):
    ...
    for index in range(5):
        map_label = single_disease_map_label(disease_map_size[index], RF_list[index],
...
def valid_data_preprocessing(origin_img, gt_box):
    if len(gt_box) == 0:
        # 如果未检测到gt_box, 图像不变
...
def train_data_preprocessing(origin_img, gt_box):
    if len(gt_box) == 0:
